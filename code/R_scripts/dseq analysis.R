...
combined <- combined[rowSums(combined >= 10) > 0, ]
...
column_data <- data.frame(row.names = colnames(combined),
...
# Save to CSV
write.csv(output_df, file = "top_20.csv", row.names = FALSE)
...
# Save to CSV
write.csv(output_df, file = "top_20_cog.csv", row.names = FALSE)
